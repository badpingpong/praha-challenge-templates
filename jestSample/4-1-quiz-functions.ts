...
class Calculator {
  public divide = (top:{a: number, b: number}, bottom: {a: number, b: number}): number => {
    const sumResult = this.add(top.a, top.b);
    const subResult = this.subtract(bottom.a, bottom.b);
    return sumResult / subResult;
  }
  public multiply = (multiplicand:{ a: number, b: number} , multiplier: {a: number, b: number}): number => {
    return this.add(multiplicand.a,multiplicand.b) * this.add(multiplier.a, multiplier.b);
  }
...
export const calculate = async (numPair1: {a:number, b: number}, numPair2: {a: number, b: number}) => {
